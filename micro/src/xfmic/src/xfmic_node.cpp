...
pthread_t id_aiui = 0;              // thread aiui process
int audio_stat = MSP_AUDIO_SAMPLE_FIRST;

const char *comm[7]={"我们可以做朋友吗","停止","前进","你会跳舞吗","你有什么本领","我可以和你握个手吗","给我唱首歌吧"};
	...
	for(j=0;j<7;j++){
		if(strcmp(rslt,comm[j])==0){
			switch(j){
				...
				case 6:command.data ="music";break;
			}
			if (command.data != ""){
...
thread of aiui
*/
void *thread_aiui_proc(void *args)
{
	// open audio file for save
	//int fd = 0;Penguin_AIUI
	...
	*/
	/*
	sprintf(save_path, "audio/aiui.pcm");
	if (strlen(save_path)) {
		// Open
	...
	*/
	...
	if (aiui_flag)
	{
		...
		*/
		cout<<"Hello! My name is Penguin AIUI，I'm listening to you (press enter to exit)....."<<endl;
...
{
	int         ret       = MSP_SUCCESS;
	const char *lgi_param = "appid = 5c7e38af,work_dir = .";
